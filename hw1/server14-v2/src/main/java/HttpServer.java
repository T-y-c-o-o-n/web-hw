...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String uri = URLDecoder.decode(request.getUri(), Charset.defaultCharset().name());

        if ("/".equals(uri)) {
            uri = "/index.html";
        } else if ("/images".equals(uri) || "/images/".equals(uri)) {  // THINK ABOUT IT
        ...
        if (file.isFile()) {
            ...
            if (etag != null && Integer.parseInt(etag) == Arrays.hashCode(body)) {
